...
class PyRobot:
    ...
    def __init__(self, client_id: int, client_mdp: str, trading_serveur: str, leverage: float, tickers: list):
    ...
    def create_entry_trades(self, dict_pos: dict):
        positions = [pos.symbol for pos in mt5.positions_get()]
        for ticker in dict_pos.keys():
            if ticker not in positions:
                order = {
                    ...
                    "type" : mt5.ORDER_TYPE_BUY if dict_pos[ticker] == 1 else mt5.ORDER_TYPE_SELL,
                    "price" : mt5.symbol_info_tick(ticker).ask if dict_pos[ticker] == 1 else mt5.symbol_info_tick(ticker).bid
                }

                mt5.order_send(order)

    def create_close_trades(self, all_preds):
        volumes = {}
        tickets = {}
        positions = mt5.positions_get()
        for position in positions:
        ...
        for ticker in pos_time.keys():
            if (pos_time[ticker]['PosType'] != sign(all_preds[ticker])):
                order = {
                    "action": mt5.TRADE_ACTION_DEAL,
    ...
    def leverage_to_volume(self, ticker):
        ...
        volume = self.leverage * account / len(self.tickers) / lot_value
